...
def parse_args():
    ...
    parser.add_argument(
        '-c', '--colors', help='Number of colors for the chart (default: 20)', type=int, default=20)
    parser.add_argument(
        '-s', '--stitches', help='Number of stitches in X axis (default: 100)', type=int, default=100)
    parser.add_argument(
        '-p', '--pattern', help='Specific alphabet or number of the pattern (default: all patterns)', type=str,
        default='all')
    parser.add_argument(
...
def prep_svg(im, pattern, palette, args, svg_cell_size=10, size=40):
    ...
    for row in pattern:
        for color_index in row:
            col_sym.add_rect(palette, color_index, x, y, svg_cell_size, args.pattern)
            blw_nsy.add_rect(palette, color_index, x, y, svg_cell_size, args.pattern)
    ...
    key.prep_for_drawing(size * 20, size * len(palette))
    x = y = 0
    for i in range(len(palette)):
        n_stitch = sum(x.count(i) for x in pattern)
        key.add_key_colour(x, y, size, i, palette[i], n_stitch)
        y += size
    return (col_sym, blw_nsy, col_nsy, key)


def save_svg(im, pattern, palette, args):
    col_sym, blw_nsy, col_nsy, key = prep_svg(im, pattern, palette, args)
